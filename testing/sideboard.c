...
volatile static GPIO_PinState pump1_inc_state = GPIO_PIN_SET; // Default HIGH (idle)
volatile static GPIO_PinState pump2_inc_state = GPIO_PIN_SET; // Default HIGH (idle)
//pump signal
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	TickType_t currentTime = xTaskGetTickCount();
		...
		if(GPIO_Pin == GPIO_PIN_1){
			pump1_inc_state = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_1);
		}else if(GPIO_Pin == GPIO_PIN_15){
      pump2_inc_state = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_15);
    }
...
void func_pump1(void *argument)
{
  ...
  for(;;)
  {
	  if(pump1_status){
		  ...
		  if (pump1_inc_state == GPIO_PIN_RESET) {
              pump1_volume++;
              pump1_inc_state = GPIO_PIN_SET;
      }
    }else{
	  HAL_GPIO_WritePin(GPIOC,GPIO_PIN_8,GPIO_PIN_SET);
    }
...
void func_pump2(void *argument)
{
  ...
  for(;;)
  {
	  if(pump2_status){
		  ...
		  if (pump2_inc_state == GPIO_PIN_RESET) {
              pump2_volume++;
              pump2_inc_state = GPIO_PIN_SET;
      }
